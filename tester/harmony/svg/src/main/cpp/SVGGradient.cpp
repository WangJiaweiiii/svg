...
void SvgGradient::OnDraw(OH_Drawing_Canvas *canvas) {
    LOG(INFO) << "hzj SvgGradient::OnDraw";
    LOG(INFO) << "hzj SvgGradient x1:" << gradientAttr_.gradient.GetLinearGradient().x1->Value()
        << " y1:" << gradientAttr_.gradient.GetLinearGradient().y1->Value()
        << " x2:" << gradientAttr_.gradient.GetLinearGradient().x2->Value()
        << " y2:" << gradientAttr_.gradient.GetLinearGradient().y2->Value();
    for (auto grad: gradientAttr_.gradient.GetColors()) {
        LOG(INFO) << "hzj SvgGradient gradient color: " << std::setprecision(10) << grad.GetColor().GetValue();
    }
//     LOG(INFO) << "hzj SvgGradient gradientUnits: " << gradientAttr_.gradient.GetGradientUnits();
    for (auto grad: gradientAttr_.gradient.GetGradientTransform()) {
        LOG(INFO) << "hzj SvgGradient gradientTransform: " << grad;
    }
